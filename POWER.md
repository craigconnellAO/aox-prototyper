---
name: "ao-designmd"
displayName: "AO designMD"
description: "AO design-system-strict prototyping kit: locked tokens/components/brand rules, per-project discovery/product/design templates, a worked Switch24 example, the Figma bridge and ideation workflows, and the impeccable frontend-craft skill."
keywords: ["ao", "ao.com", "design system", "strata", "design.md", "figma", "prototype", "switch24", "impeccable", "ideation", "steering"]
author: "AO Design Systems"
---

# AO designMD

A shareable Kiro Power that packages the AO (ao.com) design-system prototyping kit — the same rules, templates, and Figma workflow used to build Switch24 — for any designer or team to install and use on a new project.

## What this Power gives you

- **Locked steering** (`steering/`) — the AO design system (`design.md`: tokens, typography, components, patterns, anti-patterns) and brand guidelines (`brand.md`). Always active once installed; this is what makes generated output token-accurate and brand-accurate instead of guessed.
...
AO UX/product designers working with Kiro (or Claude Code, which reads the same `steering/`, `skills/`, and template files) who want AI-generated prototypes and Figma builds to be strict to the AO design system on the first pass, not after several correction rounds.
...
1. **Confirm the steering is active.** Ask Kiro "what design system am I working under?" — it should describe the AO token/component rules from `steering/ao-design-system.md`. If it doesn't, the Power isn't installed correctly.
...
- Always: `steering/ao-design-system.md`, `steering/design.md`, `steering/brand.md` — these are `inclusion: always`.
- Building or editing any prototype screen → `steering/ao-design-system.md` §Protocols (resolve-before-build, verify-or-flag)
