...
A Kiro Power for building AO (ao.com) design-system-strict prototypes and Figma builds — with an AI that pulls real tokens, real components, and real brand rules instead of guessing.
...
steering/                   LOCKED — always active once installed
  ao-design-system.md         Workspace rules + the four insights-derived protocols
  design.md                   AO design system: tokens, typography, components, patterns, anti-patterns
  brand.md                    AO brand guidelines: voice, colour, logo, graphic language
...
- Use real AO colour tokens — never raw hex
...
- Follow mobile-first layout with AO's breakpoint scale
...
## Relationship to the wider AO designMD project

This Power is the shareable, stripped-down product of a longer internal experimentation process (originally "AO Figma Make Kit," through v2 and v3). The full experimentation history — dead-end prototypes, every ideation variant, the build tooling that assembles `design.md` from its source `kit/*.md` files — lives outside this package, in the maintainer's working repositories. This Power ships only what a new project needs to start.

---

**v1.0** · 2026-07-25 · AO Design Systems
