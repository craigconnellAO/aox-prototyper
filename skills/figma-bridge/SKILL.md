...
This skill carries the **method** — the conventions that hold for any project. Project-specific facts (which component-library keys map to which elements, which Figma file/page to push to, the running push log) belong in that project's own `FIGMA-BRIDGE.md`, following the pattern in `example-switch24/FIGMA-BRIDGE.md`. Read the project's file first if one exists; create it from that example's structure if this is a new project's first push.

## Before every push

1. **Confirm the connection is live** — the Figma MCP and Desktop Bridge plugin must both be running and the working file + component-library file must be connected (so `importComponentByKeyAsync` resolves). See `steering/ao-design-system.md` Protocol 3 (resilient-Figma) — don't start a heavy build without checking this first.
2. **Resolve components before drawing primitives.** Search the design system library for an existing component before hand-drawing a card, notice, or button. Record in the project's `FIGMA-BRIDGE.md` when no library component exists for something — that becomes the standing "draw as primitive" list for next time.
...
4. **Fonts have exact Figma names that differ from CSS.** A bold family with a space in its name (e.g. `Smiley Face` not `SmileyFace`) will silently fail to load if you use the CSS font-family string verbatim. Verify each font's exact Figma `fontName` object once per project and record it in that project's `FIGMA-BRIDGE.md` font map.
...
This exists because Figma Desktop Bridge disconnects, 403 auth errors, and session limits have repeatedly interrupted builds mid-flow and lost progress (see `steering/ao-design-system.md` Protocol 3).
